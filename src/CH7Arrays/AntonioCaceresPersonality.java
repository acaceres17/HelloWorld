...
public class AntonioCaceresPersonality {
    public static final String DATA_FILE = "personality.txt";
    ...
    public static int[] percent(int[][]asbs){
        //creates an array of percentages
        int[] percent=new int[4];
       //simple math to find percent of whole
        for(int i=0;i<4;i++)
            percent[i] = (int)(asbs[i][1] /(double)(asbs[i][1]+asbs[i][0])  * 100.0);
        return percent;
    }
    //this method determines the personality category and returns it as a String
    public static String findCategory(int[] percents){
        //creates the String to store the personality category based on the percentages
        String category="";
        if(percents[0]<50)
            category+="E";
